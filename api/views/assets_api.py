...
def service_list(request,format=None):
    ...
    if request.method == 'GET':     
        ...
        try:  
            ...
            recordAssets(user=str(request.user),content="添加业务类型名称：{service_name}".format(service_name=request.data.get("service_name")),type="service",id=serializer.data.get('id'))
        except Exception as ex:
...
def service_detail(request, id,format=None):
    ...
    if request.method == 'GET':
        ...
        if serializer.is_valid():
            serializer.save()
            recordAssets(user=str(request.user),content="修改业务类型为：{old_name} -> {service_name}".format(old_name=old_name,service_name=request.data.get("service_name")),type="service",id=id)
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def asset_server_detail(request, id,format=None):
    ...
    if request.method == 'GET':
        ...
        if serializer.is_valid():
            serializer.save()
            recordAssets(user=str(request.user), content="修改服务器资产：{ip}".format(ip=snippet.ip), type="server",
                               id=id)
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            snippet.delete()
            recordAssets(user=str(request.user),content="删除服务器资产：{ip}".format(ip=snippet.ip),type="server",id=id)
        except Server_Assets.DoesNotExist:
            pass       
        return Response(status=status.HTTP_204_NO_CONTENT)         
