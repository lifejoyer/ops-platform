...
#         serverList = Server_Assets.objects.all()
        ...
        try: 
            project = Project_Config.objects.create(
                                                    ...
                                                    project_name = proAssets.project_english_name,
            ...
            recordProject(project_user=str(request.user),project_id=project.id,project_name=project.project_name,project_content="添加项目")
        except Exception as ex:
        ...
        if ipList:
            for sid in ipList.split(','):
                try:
                    Project_Number.objects.create(dir=request.POST.get('dir'),server=sid,project=project)
                except Exception as ex:
                    project.delete()
                    logger.error(msg="部署项目添加失败: {ex}".format(ex=ex))
                    return JsonResponse({'msg':"部署项目添加失败: {ex}".format(ex=ex),"code":500,'data':[]})            
        ...
        if ipList:
            ...
            for sid in ipList.split(','):
                try:
                    ...
                    if sid not in tagret_server_list:
                        Project_Number.objects.create(dir=request.POST.get('dir'),
                                                      server=sid,
                                                      project=project)
                    elif sid in tagret_server_list and request.POST.get('dir'):
                        try:
                            Project_Number.objects.filter(project=project,server=sid).update(dir=request.POST.get('dir'))
                        except Exception as ex:
...
def deploy_init(request,pid):      
    if request.method == "POST": 
        ...
        if result[0]>0: return JsonResponse({'msg':result[1],"code":500,'data':[]})
            ...
            recordProject(project_user=str(request.user),project_id=project.id,project_name=project.project_name,project_content="初始化项目")
