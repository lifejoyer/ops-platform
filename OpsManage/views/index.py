...
def index(request):
    ...
    userList = Deploy_Record.objects.raw('''SELECT t2.id,COUNT(t2.id) as count,t2.user as user,t1.first_name AS username  FROM auth_user t1,opsmanage_deploy_record t2 WHERE t1.username = t2.user GROUP BY t2.user;''')
    # '''SELECT id,count(id) as count,user as username FROM opsmanage_deploy_record GROUP BY user;'''
    ...
    for user in userList:
        ...
        for startTime in dateList:            
            sql = """SELECT id,IFNULL(count(0),0) as count from opsmanage_deploy_record WHERE 
                    ...
                    and user='{user}'""".format(startTime=startTime,user=user.user)
    ...
    for ms in monthList:
        ...
        for user in userList:
            sql = """SELECT id,IFNULL(count(0),0) as count from opsmanage_deploy_record WHERE date_format(create_time,"%%Y%%m%%d") >= {startTime} and 
                    date_format(create_time,"%%Y%%m%%d") <= {endTime} and user='{user}'""".format(startTime=startTime,endTime=endTime,user=user.user)
            userData = Deploy_Record.objects.raw(sql)
    ...
    userList = Deploy_Record.objects.raw("SELECT t2.id,t1.username,t2.user,t1.first_name AS order_user  FROM auth_user t1,opsmanage_deploy_record t2 WHERE t1.username = t2.user GROUP BY t2.user;")
